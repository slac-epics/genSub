...
static long findField( int flag, struct dbAddr *paddr, long *no_elements, long nNew )
{
  ...
  switch( fieldIndex )
  {
    ...
    case genSubRecordT:
      ...
      {
	    /*
		 * Note: genSubRecord.not causes an error when genSubRecord.h is
		 * included from c++ code unless you compile with -fno-operator-names
		 * genSubRecord.h is generated by EPICS, so unfortunately this warning
		 * can't go there where it would be more usefull.
		 */
